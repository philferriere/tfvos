...
def backbone(inputs, segnet_stream='fstream'):
    """Defines the backbone network (same as the OSVOS network, with small variation in input size)
    Args:
...
def preprocess_labels(label):
    ...
    Args:
        Label (W,H)  or (W,H,1) corresponding to the input image numpy array
    Returns:
...
def load_vgg_imagenet(ckpt_path, segnet_stream='fstream'):
...
def parameter_lr(segnet_stream='fstream'):
...
def _train(dataset, initial_ckpt, supervison, learning_rate, logs_path, max_training_iters, save_step, display_step,
           global_step, segnet_stream='fstream', iter_mean_grad=1, batch_size=1, momentum=0.9, resume_training=False, config=None, finetune=1,
           test_image_path=None, ckpt_name='fstream'):
    """Train OSVOS
    Args:
    ...
    segnet_stream: Binary segmentation network stream; either "appearance stream" or "flow stream" ['astream'|'fstream']
    ...
    # 'astream' to be the concatenation of the current frame I<sub>t</sub> and the warped prediction of the previous
    ...
    # frame I<sub>t</sub>.
    # We chose the input of the flow stream 'fstream' to be the concatenation of the magnitude of the flow field from
    ...
    with tf.Session(config=config) as sess:
        ...
        while step < max_training_iters + 1:
            ...
            for _ in range(0, iter_mean_grad):
                batch_image, batch_label = dataset.next_batch(batch_size, 'train', segnet_stream)
                image = preprocess_img(batch_image[0], segnet_stream)
            ...
            if step % save_step == 0:
                if test_image_path is not None:
                    curr_output = sess.run(img_summary, feed_dict={input_image: preprocess_img(test_image_path, segnet_stream)})
...
def train_parent(dataset, initial_ckpt, supervison, learning_rate, logs_path, max_training_iters, save_step,
                 display_step, global_step, segnet_stream='fstream', iter_mean_grad=1, batch_size=1, momentum=0.9, resume_training=False,
                 config=None, test_image_path=None, ckpt_name='fstream'):
...
def test(dataset, checkpoint_file, result_path, segnet_stream='fstream', config=None):
    """Test one sequence
    Args:
        dataset: Reference to a Dataset object instance
    ...
    with tf.Session(config=config) as sess:
        ...
        for frame in range(0, dataset.get_test_size()):
            img, curr_img = dataset.next_batch(batch_size, 'test', segnet_stream)
            curr_frame = os.path.basename(curr_img[0]).split('.')[0] + ".png" # Phil: was: curr_frame = curr_img[0].split('/')[-1].split('.')[0] + '.png'
            image = preprocess_img(img[0], segnet_stream)
